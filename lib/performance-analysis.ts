...
export function analyzeBottlenecks(metrics: PerformanceMetrics): string[] {
  ...
  if (timings.configFetch > thresholds.configFetch) {
    bottlenecks.push(`Config fetch is slow (${timings.configFetch}ms). Consider edge caching or CDN.`);
  }
  
  if (timings.sessionCreation > thresholds.sessionCreation) {
    bottlenecks.push(`Session creation is slow (${timings.sessionCreation}ms). Check ElevenLabs API latency.`);
  }
  
  if (timings.websocketConnection > thresholds.websocketConnection) {
    bottlenecks.push(`WebSocket connection is slow (${timings.websocketConnection}ms). Check network conditions.`);
  }
  
  if (timings.firstMessage > thresholds.firstMessage) {
    bottlenecks.push(`Time to first message is slow (${timings.firstMessage}ms). Check agent configuration.`);
  }
 ...
 */
export function getDeviceInfo(): { device: 'mobile' | 'desktop'; browser: string; networkType?: string } {
...
export function getPerformanceRecommendations(metrics: PerformanceMetrics): string[] {
  ...
  if (device === 'mobile') {
    ...
    if (networkType && ['slow-2g', '2g', '3g'].includes(networkType)) {
      recommendations.push('Detected slow network. Consider offline-first approach or service workers');
    }
  }
